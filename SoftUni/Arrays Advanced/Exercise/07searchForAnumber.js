...
// [5, 2, 3, 4, 1, 6],
// [5, 2, 3]	                Number 3 occurs 1 times.	    First, we take 5 elements from the array. Delete the first 2 elements. Then we search for the number 3.

// [7, 1, 5, 8, 2, 7],
// [3, 1, 5]	                Number 5 occurs 1 times.	


function searchForAnumber(firstArray, secondArray) {

    let count = 0;
    let newArray = firstArray.slice(0, secondArray[0]);
    //console.log(newArray);
    for (let i = 0; i < secondArray[1]; i++) {
        newArray.shift();
    }
    //console.log(newArray);
    for (let i = 0; i < newArray.length; i++) {
        if (newArray[i] === secondArray[2]) {
            count++;
        }
    }
